...
OUTPUT = ROOT / "outputs/professor_report"
GENERATED_AT = "2026-08-21T02:45:00+09:00"
...
def main() -> None:
    ...
    g3 = pd.read_csv(ROOT / "outputs/architecture_chum_g3/G3_CELL_SUMMARY.csv")
    g3_rows = g3.loc[
        (g3["mode"] == "loo_sample")
        & g3.fault_id.isin([4, 19, 25, 26])
        & g3.stable_material
    ].copy()
    g3_rows["fault_channel"] = g3_rows.apply(
        lambda row: f"Fault {int(row.fault_id)} / XMV{int(row.channel)}", axis=1
    )
    g3_rows["architecture_label"] = g3_rows.architecture.map(
    ...
    hai_global = hai.loc[hai.label == "attack"].copy()
    hai_global["variant_order"] = hai_global.variant.map({"F0": 0, "F0-C": 1, "F1": 2})
    hai_global = hai_global.sort_values("variant_order")
    hai_chart = hai_global.melt(
        id_vars=["variant"],
    ...
    target_delta = pd.read_csv(
        ROOT
        / "outputs/hai_external_validation_v2/HAI_EXTERNAL_TARGET_DELTA_SUMMARY.csv"
    )
    target_delta["target_group"] = target_delta.target_class.map(
        {
            "control_only": "Control-only target (31 events)",
            "mixed_control_sensor": "Control + sensor target (19 events)",
        }
    )
    target_delta = target_delta[
        [
            "target_group",
            "contrast",
            "mean_delta_detected_ratio",
            "positive_seeds_detected_ratio",
            "mean_delta_censored_delay",
            "positive_seeds_delay",
        ]
    ]
    ...
    validation_compact = validation.loc[
    ...
    sources = [
        {
            "id": "src-g3",
            "label": "Architecture Conditional CHUM G3 raw summaries",
            "path": "outputs/architecture_chum_g3/G3_CELL_SUMMARY.csv",
            "query": {
                "engine": "duckdb",
                "language": "sql",
                "sql": "SELECT * FROM read_csv_auto('outputs/architecture_chum_g3/G3_CELL_SUMMARY.csv') WHERE mode = 'loo_sample' AND fault_id IN (4, 19, 25, 26) AND stable_material = true",
                "description": "Select distribution-preserving primary CHUM cells that passed every final gate.",
                "tables_used": ["outputs/architecture_chum_g3/G3_CELL_SUMMARY.csv"],
                "filters": ["mode=loo_sample", "fault_id in 4,19,25,26", "stable_material=true"],
                "metric_definitions": ["mean_delta_auroc = original fault AUROC minus AUROC after one-channel replacement"],
            },
        },
        {
            "id": "src-ig",
            "label": "Integrated Gradients and CHUM rank agreement",
            "path": "outputs/integrated_gradients_baseline/IG_CHUM_CLASS_SUMMARY.csv",
            "query": {
                "engine": "duckdb",
                "language": "sql",
                "sql": "SELECT * FROM read_csv_auto('outputs/integrated_gradients_baseline/IG_CHUM_CLASS_SUMMARY.csv')",
                "description": "Read fault-class attribution agreement summaries computed from five seeds.",
                "tables_used": ["outputs/integrated_gradients_baseline/IG_CHUM_CLASS_SUMMARY.csv"],
                "metric_definitions": ["top1_agreements counts architecture-fault cells with identical highest-ranked XMV under IG and CHUM"],
            },
        },
        {
            "id": "src-hai",
            "label": "Corrected HAI 21.03 v2 external validation",
            "path": "outputs/hai_external_validation_v2/HAI_EXTERNAL_AGGREGATE.csv",
            "query": {
                "engine": "duckdb",
                "language": "sql",
                "sql": "SELECT * FROM read_csv_auto('outputs/hai_external_validation_v2/HAI_EXTERNAL_AGGREGATE.csv') WHERE label = 'attack'",
                "description": "Select three-seed global-label means for F0, F0-C, and F1 from the corrected v2 run.",
                "tables_used": ["outputs/hai_external_validation_v2/HAI_EXTERNAL_AGGREGATE.csv"],
                "filters": ["label=attack", "corrected v2 only"],
                "metric_definitions": ["FPR is the fraction of global-label normal seconds that satisfy the three-consecutive-exceedance alarm rule"],
            },
        },
        {
            "id": "src-hai-targets",
            "label": "HAI 21.03 event target subgroup results",
            "path": "outputs/hai_external_validation_v2/HAI_EXTERNAL_TARGET_DELTA_SUMMARY.csv",
            "query": {
                "engine": "duckdb",
                "language": "sql",
                "sql": "SELECT * FROM read_csv_auto('outputs/hai_external_validation_v2/HAI_EXTERNAL_TARGET_DELTA_SUMMARY.csv')",
                "description": "Read paired seed deltas by official attack-target class.",
                "tables_used": ["outputs/hai_external_validation_v2/HAI_EXTERNAL_TARGET_DELTA_SUMMARY.csv"],
                "metric_definitions": ["Positive delay delta means F1 detects earlier than the named baseline"],
            },
        },
        {
            "id": "src-validation",
            "label": "Independent final evidence recomputation",
            "path": "outputs/final_evidence_validation/FINAL_EVIDENCE_CHECKS.csv",
            "query": {
                "engine": "duckdb",
                "language": "sql",
                "sql": "SELECT * FROM read_csv_auto('outputs/final_evidence_validation/FINAL_EVIDENCE_CHECKS.csv')",
                "description": "Read independent checks recomputed from raw final result tables.",
                "tables_used": ["outputs/final_evidence_validation/FINAL_EVIDENCE_CHECKS.csv"],
            },
        },
    ]

    datasets = {
        "card-g3": [{"value": 3, "total": 4}],
        "card-ig": [{"value": 7, "total": 8}],
        "card-hai-auroc": [{"value": 0.018745, "positive_seeds": 3}],
        "card-hai-etaf1": [{"value": 0.026722, "positive_seeds": 3}],
        "g3-cells": records(g3_rows),
        "ig-summary": records(ig),
        "hai-chart": records(hai_chart),
        "hai-global": records(hai_table),
        "hai-target-deltas": records(target_delta),
        "validation-checks": records(validation_compact),
    }

    cards = [
        {
            "id": "card-g3",
            "description": "분포 보존 개입과 두 아키텍처 합의를 모두 통과한 locked primary fault",
            "dataset": "card-g3",
            "sourceId": "src-g3",
            "metrics": [
                {"label": "Architecture-consensus faults (of 4)", "field": "value", "format": "number"}
            ],
        },
        {
            "id": "card-ig",
            "description": "Primary architecture-fault cells에서 IG와 CHUM의 1위 XMV 일치",
            "dataset": "card-ig",
            "sourceId": "src-ig",
            "metrics": [
                {"label": "IG / CHUM top-1 agreement (of 8)", "field": "value", "format": "number"}
            ],
        },
        {
            "id": "card-hai-auroc",
            "description": "HAI global attack label, F1 minus F0, 3-seed paired mean",
            "dataset": "card-hai-auroc",
            "sourceId": "src-hai",
            "metrics": [
                {"label": "HAI mean ΔAUROC", "field": "value", "format": "number", "signed": True}
            ],
        },
        {
            "id": "card-hai-etaf1",
            "description": "HAI global attack label, F1 minus F0, official eTaPR implementation",
            "dataset": "card-hai-etaf1",
            "sourceId": "src-hai",
            "metrics": [
                {"label": "HAI mean ΔeTaF1", "field": "value", "format": "number", "signed": True}
            ],
        },
    ]
    ...
    charts = [
        {
            ...
            "subtitle": "Mean AUROC loss after distribution-preserving XMV replacement; five seeds per architecture",
            "intent": "comparison",
            "question": "Do the same fault-channel effects recur across TCN and Transformer?",
            "rationale": "Grouped bars preserve the fault-channel pairing while exposing architecture differences.",
            "type": "bar",
            "dataset": "g3-cells",
            "sourceId": "src-g3",
            "encodings": {
        ...
        },
        {
            ...
            "subtitle": "Three-seed means; F0 sensor-only, F0-C capacity-matched sensor-only, F1 sensor plus control history",
            "intent": "comparison",
            "question": "Does control history transfer beyond TEP after capacity matching?",
            "rationale": "Grouped bars compare three same-scale performance metrics across the controlled variants.",
            "type": "bar",
            "dataset": "hai-chart",
            "sourceId": "src-hai",
            "encodings": {
    ...
    tables = [
        {
            ...
            "subtitle": "Only cells passing materiality, 5/5 seed direction, FPR, imputer-quality, and hierarchical run-CI gates",
            "dataset": "g3-cells",
            "sourceId": "src-g3",
        ...
        {
            "id": "table-ig",
            "title": "Attribution-method agreement",
            "subtitle": "Primary cells are the locked faults 4, 19, 25, and 26 across two architectures",
            "dataset": "ig-summary",
            "sourceId": "src-ig",
            "defaultSort": {"field": "median_spearman", "direction": "desc"},
            "density": "spacious",
            "layout": "full",
            "columns": [
                ...
                {"field": "top1_agreements", "label": "Top-1 matches", "format": "number"},
        ...
        },
        {
            ...
            "subtitle": "Global attack label; three-seed means and fixed validation-calibrated thresholds",
            "dataset": "hai-global",
            "sourceId": "src-hai",
            ...
            "columns": [
                ...
                {"field": "fpr_mean", "label": "FPR", "format": "percent"},
                {"field": "detected_mean", "label": "Event detection", "format": "percent"},
                {"field": "delay_mean", "label": "Censored delay (s)", "format": "number"},
            ],
        },
        {
            "id": "table-targets",
            "title": "HAI attack-target subgroup deltas",
            "subtitle": "Positive values favor F1; every event directly targets at least one control-history point",
            "dataset": "hai-target-deltas",
            "sourceId": "src-hai-targets",
            "defaultSort": {"field": "mean_delta_detected_ratio", "direction": "desc"},
            "density": "spacious",
            "layout": "full",
            "columns": [
                {"field": "target_group", "label": "Target group", "type": "text"},
                {"field": "contrast", "label": "Contrast", "type": "text"},
                {"field": "mean_delta_detected_ratio", "label": "Δ detection", "format": "percent", "movement": True},
                {"field": "positive_seeds_detected_ratio", "label": "Positive seeds", "format": "number"},
                {"field": "mean_delta_censored_delay", "label": "Earlier by (s)", "format": "number", "movement": True},
                {"field": "positive_seeds_delay", "label": "Earlier seeds", "format": "number"},
            ],
        },
        {
            "id": "table-validation",
            "title": "Evidence audit",
            "subtitle": "Selected checks recomputed from raw CSVs after all experiments completed",
    ...
    artifact = {
        ...
        "manifest": {
            ...
            "surface": "report",
            "title": TITLE,
            "description": "교수 면담용 기술 보고서: 문제의식, 실험 설계, 결과, 한계와 다음 단계",
    ...
    markdown_parts = [block["body"] for block in blocks if block["type"] == "markdown"]
    markdown_parts.insert(5, g3_rows.round(4).to_markdown(index=False))
    markdown_parts.insert(8, ig.round(4).to_markdown(index=False))
    markdown_parts.insert(11, hai_table.round(4).to_markdown(index=False))
    markdown_parts.insert(13, target_delta.round(4).to_markdown(index=False))
    (OUTPUT / "PROFESSOR_RESEARCH_REPORT_KO.md").write_text(
        "\n\n".join(markdown_parts) + "\n", encoding="utf-8"
    )
    source_notes = """# Report Source Notes

Audience: technical. Decision: whether the current work forms a defensible AI-engineering master's thesis and how to frame it for the supervising professor.

Chart map:
- G3 section: grouped bar, fault-channel by architecture, mean AUROC loss; supports architecture-level conditional-utility consensus.
- HAI section: grouped bar, F0/F0-C/F1 by AUROC/AUPRC/eTaF1; supports capacity-controlled external transfer.

IG has only two fault classes and is kept as an exact table rather than an underpowered chart. Attack-target subgroup results are also tabular because exact detection and delay deltas matter more than visual shape.

The report uses only corrected HAI v2 results. `outputs/hai_external_validation` is invalidated and excluded.
"""
    (OUTPUT / "REPORT_SOURCE_NOTES.md").write_text(source_notes, encoding="utf-8")
